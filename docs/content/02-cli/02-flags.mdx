...
Selects the output format. See the [Reporters](/latest/cli/reporters) page for details on each one.
...
Applies safe automatic fixes — rewriting mistagged `ts`/`typescript` fences to `tsx` when JSX is detected, adding `group=` tags to ungrouped continuation snippets, and writing framework `jsxImportSource` overrides into your config. See the [--fix](/latest/cli/fix) page.
