...
Kiira checks fenced code blocks whose language is one of `ts`, `tsx`, `js`, or `jsx`, in both **`.md` and `.mdx`** files (MDX is supported out of the box — fences inside JSX components like `<Tabs>` are extracted correctly). Write your examples exactly as you normally would:
